...
#---------------------------------------------------
@app.put("/employees/{employee_id}", response_model=EmployeePublic)
def update_employee(employee_id: int, updated_emp: EmployeeCreate):
    """Fully update an employee by ID. Returns 404 if not found."""
    for emp in employees:
        if emp["id"] == employee_id:
            ...
            emp["active"] = updated_emp.active
            return emp
    # If loop finished without returning, employee wasn't found
